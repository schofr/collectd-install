    ...
    while [ $# -gt 0 ]; do
        case $1 in
           ...
           --configure-only)
              ...
              shift 1 ;;
           --download-only)
              export download_only="--downloadonly"
              shift 1 ;;
           --download-dir)
              dir="$2"
              [ -n "$2" ] && $sudo mkdir -p $2/signalfx-collectd && $sudo mkdir -p $2/signalfx-collectd-plugin && export download_dir="$2" && export download_dir_opt="--downloaddir=$download_dir"
              shift 1 ;;
           --local-resources)
              rep="$2"
              [ -n "$2" ] && export local_resources="$2" && echo "Relying on local resources located at $local_resources"
              shift 1 ;;
...
install_rpm_collectd_procedure() {
    ...
    # set up local repository if local_resources specified
    if [ -n "$local_resources" ]; then
      #Set up local repositories using the local resources
      sudo /bin/su -c "cat << 'endrepo' > /etc/yum.repos.d/SignalFx-collectd-RPMs-centos-release.repo
[SignalFx-collectd]
name=SignalFx release repo
baseurl=file://$local_resources/signalfx-collectd
gpgkey=""
gpgcheck=0
endrepo
"
      $sudo chmod -R o-w+r "$local_resources"/signalfx-collectd
    fi

    #install deps
    printf "Installing Dependencies\n"
    $sudo yum $yum_flags $download_only $download_dir_opt/signalfx-collectd install $needed_deps
    ...
    #download all packages from the SignalFx repo
    if [ -n "$download_only" ]; then
      $sudo yum install -y createrepo
      printf "Downloading all available packages\n"
      $sudo yum install --skip-broken $download_only "$download_dir_opt"/signalfx-collectd $(yum --disablerepo="*" --enablerepo="SignalFx-collectd" list available | awk '{print $1}' | xargs echo)
      $sudo createrepo "$download_dir"/signalfx-collectd
    else
      #install collectd from signalfx rpm
      printf "Installing collectd\n"
      $sudo yum $yum_flags $download_only $download_dir_opt install collectd

      #install base plugins signalfx deems necessary
      printf "Installing base-plugins\n"
      $sudo yum $yum_flags $download_only $download_dir_opt install collectd-disk collectd-write_http
    fi
...
install_rpm_plugin_procedure() {
    ...
    if [ -n "$local_resources" ]; then
      #Set up local repositories using the local resources
      sudo /bin/su -c "cat << 'endrepo' > /etc/yum.repos.d/SignalFx-collectd_plugin-RPMs-centos-release.repo
[SignalFx_Collectd_Plugin]
name=SignalFx release collectd plugin repo
baseurl=file://$local_resources/signalfx-collectd-plugin
gpgkey=""
gpgcheck=0
endrepo
"
      $sudo chmod -R o-w+r "$local_resources"/signalfx-collectd-plugin
    else
    ...
    if [ -n "$download_only" ]; then
      printf "Downloading all available packages"
      $sudo yum install -y createrepo
      $sudo yum install --skip-broken $download_only "$download_dir_opt"/signalfx-collectd-plugin $(yum --disablerepo="*" --enablerepo="SignalFx_Collectd_Plugin" list available | awk '{print $1}' | xargs echo)
      $sudo createrepo "$download_dir"/signalfx-collectd-plugin
    else
...
get_collectd_config() {
    if [ -n $local_resources ]; then
      tar -C "$BASE_DIR" -xzf $local_resources/install-files${test_files}.tgz 
    else
